...
/**
 * Декодирование тела BER/DER OBJECT IDENTIFIER (примитив 0x06, содержимое TLV) в список дуг ITU-T.
 * Используется для BER/DER и для PER OPEN TYPE, когда полезная нагрузка — те же октеты.
 */
inline std::optional<std::vector<std::uint32_t>> object_identifier_components(std::string_view body) {
    if (body.empty()) {
