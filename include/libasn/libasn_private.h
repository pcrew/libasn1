...
struct common_type {
    ...
    constexpr auto context_specific() const {
        return common_type(_identifier.template tagged<tag_class_enum::CONTEXT_SPECIFIC, tag_number>(), _serde);
    }

    template <auto tag_number>
    constexpr auto application() const {
        return common_type(_identifier.template tagged<tag_class_enum::APPLICATION, tag_number>(), _serde);
    }
